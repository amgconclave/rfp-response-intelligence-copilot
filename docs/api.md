...
Returns the Buyer Proposal Release Room. It composes buyer workflow state, replay checkpoints, agent council handoffs, decision provenance, submission certification, review gate, proposal observability, and provider resilience into one operator-facing decision board with HITL queue, durable checkpoints, provider route, trace coverage, eval assertions, proof commands, limitations, and trace ID.

```bash
curl -X GET "http://127.0.0.1:8000/proposal/release-room" \
  -H "X-API-Key: local-demo-key"
```

### `POST /proposal/release-room-pack`

Writes Markdown and JSON artifacts under ignored `storage/proposal_release_room/` by default.
...
### `POST /learning/win-loss-eval-cases`

Compiles candidate standard eval and red-team rows from the Win/Loss Learning response. The response includes typed case candidates, dataset patch metadata, governance summary, traceable compile-state transitions, owner review queue, proof commands, limitations, and trace ID. It does not modify checked-in eval fixtures.
...
Optional body fields include `learning_response`, `eval_dataset_path`, `red_team_dataset_path`, `outcomes_fixture_path`, inline `outcomes`, and `max_cases_per_type`.

### `POST /learning/win-loss-eval-case-pack`

Writes a Win/Loss Eval Case Compiler Pack under ignored `storage/win_loss_eval_cases/` by default, including Markdown, JSON, candidate standard eval JSON, and candidate red-team JSON artifacts.
...
The pack includes candidate eval rows, candidate red-team rows, dataset patch metadata, owner review queue, trace spans, proof commands, limitations, embedded compiler response JSON, Markdown, JSON, and trace ID.
