...
tool-trust:
	$(PYTHON) -c "import httpx; r=httpx.get('http://127.0.0.1:8000/proposal/tool-trust-registry', headers={'X-API-Key': 'local-demo-key'}, timeout=30).json(); print({'status': r['status'], 'tools': len(r['trust_registry']), 'approvals': len(r['human_approval_queue'])})"

tool-trust-pack:
	$(PYTHON) -c "import httpx; p=httpx.post('http://127.0.0.1:8000/proposal/tool-trust-pack', headers={'X-API-Key': 'local-demo-key'}, json={}, timeout=30).json(); print({'artifact': p['artifact_path'], 'json': p['json_artifact_path']})"
...
evidence-room:
	$(PYTHON) -c "import httpx; m=httpx.get('http://127.0.0.1:8000/proposal/evidence-room', headers={'X-API-Key': 'local-demo-key'}, timeout=60).json(); print({'status': m['status'], 'present': m['summary']['present_item_count'], 'missing_required': m['summary']['missing_required_count'], 'hash_coverage': m['summary']['hash_coverage_ratio']})"

evidence-room-pack:
	$(PYTHON) -c "import httpx; p=httpx.post('http://127.0.0.1:8000/proposal/evidence-room-pack', headers={'X-API-Key': 'local-demo-key'}, json={}, timeout=60).json(); print({'artifact': p['artifact_path'], 'json': p['json_artifact_path']})"

submission-escrow:
	$(PYTHON) -c "import httpx; e=httpx.get('http://127.0.0.1:8000/proposal/submission-escrow', headers={'X-API-Key': 'local-demo-key'}, timeout=60).json(); print({'status': e['status'], 'score': e['custody_score'], 'records': e['summary']['record_count'], 'signoff': e['summary']['owner_signoff_count']})"

submission-escrow-pack:
	$(PYTHON) -c "import httpx; p=httpx.post('http://127.0.0.1:8000/proposal/submission-escrow-pack', headers={'X-API-Key': 'local-demo-key'}, json={}, timeout=60).json(); print({'artifact': p['artifact_path'], 'json': p['json_artifact_path']})"
