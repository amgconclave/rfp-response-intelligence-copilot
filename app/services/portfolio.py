...
class PortfolioService:
    ...
    def evidence_index(self, trace_id: str) -> PortfolioEvidenceIndexResponse:
        ...
        return PortfolioEvidenceIndexResponse(
            ...
            artifact_roots={
                ...
                "procurement_risk_desk": str((self.settings.storage_dir / "procurement_risk_desk").resolve()),
                "procurement_risk_decisions": str((self.settings.storage_dir / "procurement_risk_decisions").resolve()),
                "procurement_exception_monitor": str((self.settings.storage_dir / "procurement_exception_monitor").resolve()),
                "review_boards": str((self.settings.storage_dir / "review_boards").resolve()),
                "reviewer_signoffs": str((self.settings.storage_dir / "reviewer_signoffs").resolve()),
    ...
    def _skills(self) -> list[PortfolioEvidenceSkill]:
        return [
            ...
            self._skill(
                ...
                "Procurement risk desk for legal, pricing, data residency, insurance, and implementation review",
                [
                    "Packet-level risk detection",
                    "Owner routing",
                    "Evidence gaps and citations",
                    "Markdown/JSON risk desk pack",
                ],
                ["/procurement/risk-desk", "/procurement/risk-desk-pack"],
                ["app/services/procurement_risk_desk.py"],
                ["tests/test_procurement_risk_desk.py", "python scripts\\dashboard_smoke.py"],
                ["storage/procurement_risk_desk/*.md", "storage/procurement_risk_desk/*.json"],
                ["python -m app.demo"],
                [
                    "sample_data/customer_contract_terms.md",
                    "sample_data/pricing_notes.md",
                    "sample_data/implementation_guide.md",
                ],
            ),
            self._skill(
                "procurement-risk-decision-ledger",
                "Human-in-the-loop owner decision ledger for procurement risk desk release gates",
                [
                    "Owner decision records",
                    "Exception and conditional-release states",
                    "Durable checkpoint metadata",
                    "Trace analysis and governance gates",
                    "Markdown/JSON decision artifacts",
                ],
                ["/procurement/risk-decision-ledger", "/procurement/risk-decision-pack"],
                ["app/services/procurement_risk_decisions.py"],
                ["tests/test_procurement_risk_decisions.py", "python scripts\\dashboard_smoke.py"],
            ...
            self._skill(
                ...
                "Expiry and evidence monitor for procurement exceptions and conditional approvals",
                [
                    "Decision ledger replay",
                    "Exception expiry controls",
                    "Evidence hold routing",
                    "Checkpointed state transitions",
                    "Markdown/JSON monitor artifacts",
                ],
                ["/procurement/exception-monitor", "/procurement/exception-monitor-pack"],
                ["app/services/procurement_exception_monitor.py"],
                ["tests/test_procurement_exception_monitor.py", "python scripts\\dashboard_smoke.py"],
                ["storage/procurement_exception_monitor/*.md", "storage/procurement_exception_monitor/*.json"],
                ["python -m app.demo"],
                [
                    "app/services/procurement_risk_decisions.py",
                    "sample_data/customer_contract_terms.md",
            ...
            self._skill(
                ...
                "Durable reviewer signoff readiness ledger with governance gates and HITL queue",
                [
                    "Role-by-role signoff readiness records",
                    "Outstanding blocker and decision-comment traceability",
                    "Governance policy gates",
                    "Human review queue",
                    "Markdown/JSON signoff artifacts",
                ],
                ["/rfp/reviewer-signoff-ledger", "/rfp/reviewer-signoff-pack"],
                ["app/services/reviewer_signoff.py"],
                ["tests/test_reviewer_signoff.py", "python scripts\\dashboard_smoke.py"],
                ["storage/reviewer_signoffs/*.md", "storage/reviewer_signoffs/*.json"],
                ["make reviewer-signoff-pack", "python -m app.demo"],
                ["docs/api.md", "README.md"],
            ),
            self._skill(
                "bid-no-bid-roi",
            ...
            self._skill(
                ...
                "Governed accepted-answer reuse library",
                [
                    "Accepted-answer snippets converted into governed reusable response language",
                    "Owner, expiry, approval status, and reuse decision per snippet",
                    "Citation lineage checks against local evidence files",
                    "Markdown/JSON library pack for reviewer handoff",
                ],
                ["/rfp/answer-reuse-library", "/rfp/answer-reuse-library-pack"],
                ["app/services/answer_reuse_library.py", "app/services/customer_intelligence.py"],
                ["tests/test_answer_reuse_library.py", "python scripts\\dashboard_smoke.py"],
                ["storage/answer_reuse_library/*.md", "storage/answer_reuse_library/*.json"],
                ["make answer-reuse-pack", "python -m app.demo"],
                ["sample_data/approved_responses.json", "docs/api.md"],
            ),
            self._skill(
    ...
    def _proof_commands(self) -> list[str]:
        return [
            ...
            "rg \"reviewer-signoff|Reviewer Signoff|reviewer_signoffs|signoff ledger\" app dashboard docs README.md tests Makefile",
