...
curl -X POST "http://127.0.0.1:8000/proposal/review-gate-pack" -H "X-API-Key: local-demo-key" -H "Content-Type: application/json" -d "{}"
```
...
The Risk Decision Ledger tab converts risk desk rows into owner decisions, exception controls, conditional-release state, durable checkpoint metadata, trace spans, and auditable `storage/procurement_risk_decisions/` artifacts:
...
- Answer Reuse Evaluation via `POST /rfp/answer-reuse-eval` and `POST /rfp/answer-reuse-eval-pack`, writing Markdown/JSON under `storage/answer_reuse_evals/` with deterministic eval cases, policy experiment comparison, trace spans, owner queues, proof commands, and limitations.
...
- Reviewer Trace Reconciliation via `POST /rfp/reviewer-trace-reconciliation` and `POST /rfp/reviewer-trace-reconciliation-pack`, writing Markdown/JSON under `storage/reviewer_reconciliation/` with cross-artifact consistency findings, trace spans, governance gates, shared-state checks, and reviewer follow-ups.
...
- Proposal Readiness Drift Monitor via `POST /rfp/proposal-readiness-drift` and `POST /rfp/proposal-readiness-drift-pack`, writing Markdown/JSON under `storage/readiness_drift/` with baseline/current readiness snapshots, regression findings, conditional owner routing, state-machine workflow checkpoints, trace spans, proof commands, and limitations.
- Readiness Score Eval Packs via `POST /rfp/readiness-score-eval`, running `sample_data/readiness_score_eval_dataset.json` locally and writing Markdown/JSON under `storage/readiness_score_evals/` with score-band assertions, workflow checkpoint checks, HITL queue checks, trace analysis, experiment comparison, governance summary, proof commands, and limitations.
...
- Competitive Objection Evidence Audit via `POST /rfp/objection-audit` and `POST /rfp/objection-audit-pack`, writing Markdown/JSON under `storage/objection_audits/` with typed claim audits, evidence status, risk signals, conditional reviewer routes, traceable state transitions, eval assertions, proof commands, and limitations.
- Clarification Question Workflow via `POST /rfp/clarification-questions` and `POST /rfp/clarification-question-pack`, writing Markdown/JSON under `storage/clarification_questions/` with buyer/internal questions, owner and reviewer routes, durable workflow checkpoints, trace spans, eval assertions, proof commands, and limitations.
...
- Win/Loss Eval Case Compiler via `POST /learning/win-loss-eval-cases` and `POST /learning/win-loss-eval-case-pack`, writing Markdown/JSON plus candidate standard eval and red-team JSON under `storage/win_loss_eval_cases/` with typed case rows, dataset patch metadata, traceable compile transitions, owner review queues, proof commands, and limitations.
...
- Proposal Intelligence Review Gate via `GET /proposal/review-gate` and `POST /proposal/review-gate-pack`, writing Markdown/JSON under `storage/proposal_review_gates/` with sales, presales, compliance, and procurement review criteria, task delegations, checkpointed transitions, eval assertions, and local/mock provider boundaries.
- Buyer Proposal Release Room via `GET /proposal/release-room` and `POST /proposal/release-room-pack`, writing Markdown/JSON under `storage/proposal_release_room/` with release recommendation, decision board, HITL queue, durable checkpoints, provider route, trace coverage, eval assertions, and operator checklist.
- Buyer Proposal Evidence Room via `GET /proposal/evidence-room` and `POST /proposal/evidence-room-pack`, writing Markdown/JSON under `storage/proposal_evidence_room/` with artifact SHA-256 hashes, missing-artifact regeneration guidance, release snapshot, approval owners, endpoint provenance, and integrity controls.
- Proposal Submission Escrow via `GET /proposal/submission-escrow` and `POST /proposal/submission-escrow-pack`, writing Markdown/JSON under `storage/proposal_submission_escrow/` with hash-lock custody records, owner signoff queue, checkpointed release states, eval assertions, and local/mock provider boundaries.
...
- Evidence Freshness + Expiry Risk via `GET /evidence/freshness` and `POST /evidence/freshness-pack`, plus owner SLA escalation via `GET /evidence/freshness-sla` and `POST /evidence/freshness-sla-pack`, writing Markdown/JSON under ignored `storage/freshness_packs/` and `storage/freshness_sla/` with source age, expiry, owner, renewal, endpoint, unsupported-claim, durable workflow checkpoint, human review queue, owner SLA ledger, role crew queue, governance policy, and local trace-span signals.
...
- `POST /rfp/answer-reuse-eval`: scores governed reusable snippets with deterministic eval cases, trace spans, and threshold experiment comparison.
- `POST /rfp/answer-reuse-eval-pack`: writes a Markdown/JSON Answer Reuse Evaluation Pack under `storage/answer_reuse_evals/`.
...
- `POST /rfp/reviewer-trace-reconciliation`: reconciles the reviewer board, workflow, signoff ledger, and escalation plan for cross-artifact state mismatches.
- `POST /rfp/reviewer-trace-reconciliation-pack`: writes Markdown/JSON Reviewer Trace Reconciliation artifacts under `storage/reviewer_reconciliation/`.
...
- `POST /rfp/proposal-readiness-drift`: compares current readiness against an approved baseline snapshot and returns drift findings, owner routes, workflow state, trace spans, and governance proof commands.
- `POST /rfp/proposal-readiness-drift-pack`: writes a Markdown/JSON Proposal Readiness Drift Pack under `storage/readiness_drift/`.
- `POST /rfp/readiness-score-eval`: runs the local readiness scorer regression dataset and writes a Readiness Score Eval Pack under `storage/readiness_score_evals/`.
...
- `POST /rfp/objection-audit`: audits generated objection responses for evidence-supported claims, risky comparative/commercial/control wording, reviewer routes, and replayable state transitions.
- `POST /rfp/objection-audit-pack`: writes a Markdown/JSON Competitive Objection Evidence Audit under `storage/objection_audits/`.
...
- `POST /learning/win-loss-eval-cases`: compiles candidate standard eval and red-team rows from win/loss learning with traceable compile transitions and owner review routing.
- `POST /learning/win-loss-eval-case-pack`: writes a Markdown/JSON Win/Loss Eval Case Compiler Pack plus candidate eval/red-team JSON under `storage/win_loss_eval_cases/`.
...
- `GET /evidence/freshness-sla`: returns owner SLA ledger rows, escalation owners, endpoint impact, role crew queues, and traceable remediation checkpoints for stale or expired evidence.
- `POST /evidence/freshness-sla-pack`: writes a Markdown/JSON Evidence Freshness SLA Ledger Pack under `storage/freshness_sla/`.
...
- `GET /proposal/review-gate`: returns sales, presales, compliance, and procurement review criteria with task delegations, checkpointed transitions, eval assertions, and endpoint evidence.
- `POST /proposal/review-gate-pack`: writes a Markdown/JSON Proposal Intelligence Review Gate Pack under `storage/proposal_review_gates/`.
- `GET /proposal/release-room`: returns the buyer proposal release room with decision board rows, HITL queue, durable checkpoints, provider route, trace coverage, and release recommendation.
- `POST /proposal/release-room-pack`: writes a Markdown/JSON Buyer Proposal Release Room Pack under `storage/proposal_release_room/`.
- `GET /proposal/evidence-room`: returns the buyer proposal evidence room manifest with local artifact hashes, approval owners, release snapshot, endpoint provenance, and integrity controls.
- `POST /proposal/evidence-room-pack`: writes a Markdown/JSON Buyer Proposal Evidence Room Pack under `storage/proposal_evidence_room/`.
- `GET /proposal/submission-escrow`: returns a proposal submission escrow ledger with artifact custody rows, owner signoff routing, checkpointed release states, eval assertions, and local/provider-optional controls.
- `POST /proposal/submission-escrow-pack`: writes a Markdown/JSON Proposal Submission Escrow Pack under `storage/proposal_submission_escrow/`.
...
- `POST /rfp/clarification-questions`: returns buyer/internal clarification questions with evidence status, HITL owner/reviewer routing, workflow checkpoints, trace spans, eval assertions, and proof commands.
- `POST /rfp/clarification-question-pack`: writes a Markdown/JSON clarification handoff pack under `storage/clarification_questions/`.
...
- Answer Reuse Evaluation panel showing reusable-answer eval cases, source-overlap threshold comparison, trace spans, owner queue, and generated `storage/answer_reuse_evals/` artifact path.
- Action Plan / Handoff Board tab showing task ownership and a handoff export path.
- Deal Readiness / Executive Report tab showing readiness score, blockers, owner bottlenecks, section completeness, reviewer bottlenecks, readiness scorer eval scenarios, readiness drift routing, and generated `storage/readiness_packs/`, `storage/readiness_score_evals/`, plus `storage/readiness_drift/` artifact paths.
...
- Reviewer Collaboration tab showing reviewer assignments, decision comments, approval status, redline summary, checkpointed workflow replay, signoff ledger, escalation routing, trace reconciliation, proof commands, and generated `storage/review_boards/`, `storage/reviewer_signoffs/`, `storage/reviewer_escalations/`, plus `storage/reviewer_reconciliation/` artifact paths.
...
- Win/Loss Learning tab showing outcome count, win rate, winning evidence patterns, losing risk patterns, retrieval/eval recommendations, response guidance updates, candidate eval/red-team rows, policy activation state machine, owner review queue, rollback plan, replay backtest results, proof commands, limitations, and generated `storage/win_loss_packs/`, `storage/win_loss_eval_cases/`, `storage/win_loss_policy/`, plus `storage/win_loss_replay/` artifact paths.
...
- Evidence Freshness tab showing source freshness, renewal calendar, unsupported-claim flags, owner follow-ups, SLA ledger rows, endpoint impact, role crew queues, and generated `storage/freshness_packs/` plus `storage/freshness_sla/` artifact paths.
...
- Release Room tab showing release recommendation, decision board, HITL queue, durable checkpoints, provider route, trace coverage, eval assertions, proof commands, limitations, and generated `storage/proposal_release_room/` artifact path.
- Evidence Room tab showing artifact manifest rows, SHA-256 hashes, missing-artifact regeneration guidance, approval owners, release snapshot, integrity controls, proof commands, limitations, and generated `storage/proposal_evidence_room/` artifact path.
- Submission Escrow tab showing hash-lock custody records, owner signoff queue, checkpointed release states, eval assertions, proof commands, limitations, and generated `storage/proposal_submission_escrow/` artifact path.
