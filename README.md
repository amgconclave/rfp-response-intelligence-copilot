...
- Evidence Freshness + Expiry Risk via `GET /evidence/freshness` and `POST /evidence/freshness-pack`, writing Markdown/JSON under ignored `storage/freshness_packs/` with source age, expiry, owner, renewal, endpoint, unsupported-claim, durable workflow checkpoint, human review queue, governance policy, and local trace-span signals.
